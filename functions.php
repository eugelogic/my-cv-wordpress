<?php
/**
 * My CV functions and definitions
 *
...
function mycv_setup() {
	 ...
	 * If you're building a theme based on My CV, use a find and replace
	 * to change 'mycv' to the name of your theme in all the template files.
	 */
	...
	register_nav_menus( array(
		'menu-1' => esc_html__( 'Primary', 'mycv' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'mycv_custom_background_args', array(
...
add_action( 'after_setup_theme', 'mycv_setup' );
 ...
 */
function mycv_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'mycv_content_width', 640 );
}
add_action( 'after_setup_theme', 'mycv_content_width', 0 );
 ...
 */
function mycv_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'mycv' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'mycv' ),
...
add_action( 'widgets_init', 'mycv_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function mycv_scripts() {
	wp_enqueue_style( 'mycv-style', get_stylesheet_uri() );

	wp_enqueue_script( 'mycv-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'mycv-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
